...
import { postReport, getUserDataByUsername, getUserDataById } from './controllers/teacher';
...
router.post('/history', upload.single('file'), protect('admin'), uploadUserHistory);

router.post('/feedback', protect(), postFeedback);
router.get('/reports', protect(), getReports);

router.post('/upload-report', upload.single('file'), protect('teacher'), postReport);
